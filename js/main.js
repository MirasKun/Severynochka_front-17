...
// Три отдельных контейнера
const cardBoxStock   = document.getElementById("card-box-stock");
const cardBoxNew     = document.getElementById("card-box-new");
const cardBoxHistory = document.getElementById("card-box-history");
const countEl        = document.getElementById("count-view");
const searchInput    = document.getElementById("search_input");
...
async function loadProducts() {
  try {
    cardBoxStock.innerHTML   = '<p class="loading-text">Загрузка…</p>';
    cardBoxNew.innerHTML     = '<p class="loading-text">Загрузка…</p>';
    cardBoxHistory.innerHTML = '<p class="loading-text">Загрузка…</p>';
    ...
    // По 4 товара в каждую секцию
    renderProducts(allProducts.slice(0, 4),  cardBoxStock);
    renderProducts(allProducts.slice(4, 8),  cardBoxNew);
    renderProducts(allProducts.slice(8, 12), cardBoxHistory);

  } catch (err) {
    console.error("Ошибка загрузки товаров:", err);
    cardBoxStock.innerHTML = '<p class="error-text">Не удалось загрузить товары.</p>';
  }
}
...
// Обработчик кликов на все три контейнера
[cardBoxStock, cardBoxNew, cardBoxHistory].forEach(container => {
  container.addEventListener("click", (e) => {
    ...
    if (e.target.closest(".like-heart")) {
      ...
      const now = isFavorite(id);
      heartBtn.classList.toggle("is-favorite", now);
      heartBtn.querySelector("path").setAttribute("fill", now ? "#FF6633" : "none");
      return;
    }
